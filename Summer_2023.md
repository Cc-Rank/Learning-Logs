...
1. LeetCode 刷题：
   - 每日进度（8 / 7）
     - 简单题：`#2475`, `#2481`
     - 中等题：`#1375.二进制字符串前缀一致的次数（思维题）`, `#1226.哲学家进餐（多线程）`, `#1177.构建回文串检测（前缀异或和）`, `#1254.统计封闭岛屿的数目（DFS、BFS）`
     - 困难题：`#1483.树节点的第K个祖先（树上倍增算法）`, `#1494.并行课程II（状压DP）`

2. LeetCode 周赛
   - 按时完成 `3` 题：`#2739`, `#2740.找出分区值（思维题）`, `#2741.特别的排列（状压DP）`
   - 未按时完成：`#2742.给墙壁刷油漆（选或不选DP/背包DP）`

3. 复习 进程线程相关知识
   - Notes:[Threads](./Notes_Nowcoder/Notes_Threads.md)
...
1. 至多只允许四个哲学家同时进餐，以保证至少有一个哲学家能够进餐，最终总会释放出他所使用过的两支筷子，从而可使更多的哲学家进餐。定义信号量 `count`，只允许4个哲学家同时进餐，这样就能保证至少有一个哲学家可以就餐。
2. 仅当哲学家的左右两支筷子都可用时，才允许他拿起筷子进餐。可以利用 AND 型信号量机制实现，也可以利用信号量的保护机制实现。利用信号量的保护机制实现的思想是通过记录型信号量 `mutex` 对取左侧和右侧筷子的操作进行保护，使之成为一个原子操作，这样可以防止死锁的出现。
3. 规定奇数号的哲学家先拿起他左边的筷子，然后再去拿他右边的筷子；而偶数号的哲学家则先拿起他右边的筷子，然后再去拿他左边的筷子。按此规定，将是1、2号哲学家竞争1号筷子，3、4号哲学家竞争3号筷子。即五个哲学家都竞争奇数号筷子，获得后，再去竞争偶数号筷子，最后总会有一个哲学家能获得两支筷子而进餐。
...
1. LeetCode 刷题：
   - 每日进度（8 / 7）
     - 简单题：`#2496`
     - 中等题：`#1262.可被三整除的最大和（贪心/DP）`, `#LCP 41.黑白翻转棋（BFS）`, `#面试题 16.19 水域大小（BFS/DFS）`, `#1401.圆和矩形是否有重叠（数学）`, `#735.行星碰撞（栈模拟）`
     - 困难题：`#1595.连通两组点的最小成本（状压DP）`, `#1659.最大化网格幸福感（TODO/三进制状压DP）`

2. LeetCode 双周赛
   - 按时完成 `2` 题：`#6892`, `#6895.构造最长的新字符串（思维题）`
   - 未按时完成：`#6898.字符串连接删减字母（三维DP）`, `#6468.统计没有收到请求的服务器数目（离线、滑动窗口）`

3. LeetCode 周赛
   - 按时完成 `3` 题：`#2748`, `#6471.得到整数零需要执行的最少操作数（枚举）`, `#6910.将数组划分成若干好子数组的方式（数学）`
   - 未按时完成：`#6891.机器人碰撞（栈模拟）`
...
1. LeetCode 每日进度（9 / 9）：
   - 简单题：`#2600`
   - 中等题：`#445.两数相加II（反转链表 + 两数之和）`, `#2178.拆分成最多数目的正偶数之和（贪心）`
   - 困难题：`#2532.过桥的时间（TODO）`
   - 复习：`#1`, `#2`, `#2679`, `#167`, `#15`
...
2. LeetCode 75:
     - 队列：`#649`
     - 链表：`#2130`
     - 二叉树：`#872`, `#1448`, `#1372`, `#1161`
     - 图：`#1926`, `#994`, `#841`, `#1466`, `#399`
     - 堆/优先队列：`#2336`, `#2542`, `#2462`
     - 二分：`#2300`, `#875`
     - 动态规划：`#1137`, `#746`, `#790`
     - 位运算：`#1318`
     - 前缀树：`#1268`
...
3. LeetCode 第 352 场周赛
   - 按时完成 `2` 题：`#2760`, `#2761`
   - 未按时完成：`#2762.不间断子数组（滑动窗口）`, `#2763. 所有子数组中不平衡数字之和（TODO）`

4. LeetCode 第 353 场周赛
   - 按时完成 `3` 题：`#6451`, `#6899`, `#6912`
   - 未按时完成：`#6919.使数组中的所有元素都等于零（差分数组）`

5. 阿秀八股：1~20、20~40

6. NOTES：
   - 完成了 GAMES 101 作业五 [Whitted-Style 光线追踪](https://github.com/Cc-Rank/GAMES101/tree/main/Assignment/Assignment5) 相关笔记。
   - 完成了 GAMES 101 作业六 [BVH、SAH](https://github.com/Cc-Rank/GAMES101/tree/main/Assignment/Assignment6) 相关笔记。
   - 完成了 GAMES 101 作业五 [路径追踪](https://github.com/Cc-Rank/GAMES101/tree/main/Assignment/Assignment7) 相关笔记。
...
1. 每日刷题，周赛，刷题先缓一缓；
2. 看完并实现 Ray Tracing in One Weekend；
3. 重心向 C++ 八股转移，多看多写；
...
1. LeetCode 每日进度（9 / 7）
   - 简单题：`#2544`
   - 中等题：`931.下降路径最小和（DP）`, `#979.在二叉树中分配硬币（dfs）`
   - 困难题：`#834.树中距离之和（换根DP）`
   - 复习：`#16`, `#18`, `#1911`
   - 其他：`#1035.不相交的线（DP）`, `#419.夹板上的战舰（遍历）`

2. LeetCode 第 354 场周赛
   - 按时完成 `*` 题：`#`
   - 未按时完成：`#`

3. 八股部分：看了 CPP-Primer 第 13 章和第 15 章内容。

4. Ray Tracing in One Weekend：
   - 看完了in One Weekend，并整理了 [笔记](https://github.com/Cc-Rank/RayTracer) 。
...
1. 每日刷题，周赛；
2. 这周主要看 WebServer 相关八股，从系统调用到HTTP协议；
3. 再复习复习光栅化相关；
...
2. 八股部分：
   - 整理了智能指针 [shared_ptr](https://github.com/Cc-Rank/Learning-Logs/blob/main/HandWriting/SharedPtr.md) 相关的笔记；
